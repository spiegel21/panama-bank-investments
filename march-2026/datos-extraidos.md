...
- **[REG]** Regulatorio no auditado — dato estadístico mensual que publica la SBP (Balance de Situación / IAB). Nuevo en el corte marzo-2026.
...
> ## 📅 Corte de datos: primera publicación 2026 (marzo 2026)
>
> Esta es la **réplica con datos publicados en marzo de 2026** (primer corte del año de la SBP). Qué
> cambió frente a la versión anterior (cierre Dic-2025):
>
> - **Sistema (SBP) — ACTUALIZADO a enero 2026:** el Balance de Situación y el Informe de Actividad
>   Bancaria (IAB) más recientes publicados a marzo de 2026 son los de **enero 2026**. Son **datos
>   regulatorios/estadísticos NO auditados** (la SBP los emite periódicamente sobre reportes de los
>   bancos; no son EE.FF. auditados). Se actualizaron: inversiones/activo del SBN y CBI, IAC de cierre
>   2025, ingreso por inversiones del sistema y **las pérdidas no realizadas de la cartera AFS** (ver
>   implicaciones contables abajo). Se marcan **[REG]** (regulatorio, no auditado).
> - **Bancos (nivel entidad) — SIN CAMBIO (dato previo, Dic-2025 auditado):** a marzo de 2026 **no hay
>   EE.FF. auditados más recientes** que el cierre Dic-2025 ya cargado (5 de 6 bancos cierran en
>   diciembre y su IN-A 2025 ya está leído; Global Bank cierra en junio, su próximo auditado es
>   jun-2026, aún no publicado). Todas las cifras por banco se **arrastran de la versión anterior** y se
>   mantienen [AUD] a su fecha de origen.
>
> **Regla de este corte:** lo que la SBP alcanzó a publicar (sistema, ene-2026) se actualiza; lo que no
> tiene fuente nueva (los seis bancos) se conserva de la versión previa, como pidió el encargo.
...
3. **Peso sobre el activo — ranking real:** Banco General 28.6% (líder amplio) > Davivienda 23.4% (inflado
   por la fusión) > Banistmo 16.0% > Global 13.4% > BAC 12.3% > Scotiabank 9.7%. Sistema — banca de licencia general (SBN) **~22.3% (Ene-2026)**, subiendo desde 22.1% (Dic-2025).
...
> publica la SBP. **Corte de este reporte = Ene-2026** (primera publicación de 2026, dato regulatorio no auditado,
> con comparativo Dic-2025 y Ene-2025 en la misma fuente). Se conserva el CBI solo como referencia.

| Indicador | 2024 (Dic) | 2025 (Dic) | **Ene-2026** | Conf. |
|---|---|---|---|---|
| **Inversiones en valores neta — SBN** | 29,285.5 | 32,152.8 | **32,467.2** | [REG] |
| **Activo total neto — SBN** | 140,383.5 | 145,675.3 | **145,438.4** | [REG] |
| **% inversiones / activo — SBN (lic. general)** | 20.9% | 22.1% | **22.3%** | [EST] |
| Inversiones — var. a/a SBN | +7.0% | +9.8% | **+10.5%** (Ene-26/Ene-25) | [REG] |
| — memo: Inversiones neta CBI | 34,195.9 | 35,897.3 | **36,165.7** | [REG] |
| — memo: Activo total CBI | 156,391.5 | 163,014.7 | **163,327.9** | [REG] |
| — memo: % inversiones / activo CBI | 21.9% | 22.0% | **22.1%** | [EST] |
| — memo: Inversiones — var. a/a CBI | | | **+5.6%** | [REG] |
| Índice adecuación capital (IAC — CBI) | 15.32% | **16.27%** (cierre 4T-2025) | 16.27% | [REG] |

Notas:
- **Actualización marzo-2026:** cifras del sistema movidas de Dic-2025 a **Ene-2026** (Balance de Situación SBP,
  reportes estadísticos 2026/01, e IAB Enero-2026). Datos **regulatorios no auditados** [REG]. Dic-2025 queda como
  comparativo (y coincide con lo cargado en la versión previa: SBN 32,152.8/145,675.3; CBI 35,897.3/163,014.7).
- Sistema **dominado por crédito**: préstamos ~62% del activo; inversiones **~22.3% del activo del SBN** (2.º bloque productivo).
- **Base licencia general (SBN), Ene-2026:** inversiones 32,467.2M / activo 145,438.4M = **22.3%** (Dic-2025: 22.1%;
  Dic-2024: 20.9%). El activo total del SBN cayó levemente m/m (−0.16%) mientras las inversiones subieron (+0.98% m/m),
  por eso el ratio sube. Crecimiento de inversiones **+10.5% a/a** (Ene-26 vs Ene-25). Es la base correcta para comparar
  contra los seis bancos del perímetro (todos de licencia general). El CBI (que suma la licencia internacional) da **22.1%**
  a Ene-2026 (36,165.7M / 163,327.9M); inversiones CBI +5.6% a/a.
- **Ingreso por inversiones del sistema (CBI, Ene-2026):** +3.81% a/a (+US$4.18M). El ingreso total por intereses del
  sistema creció apenas +1.26% a/a; los préstamos +2.91%. (El "+8.5% a/a" de la versión previa era el dato a oct-2025;
  el ritmo se ha moderado con la normalización de tasas.)
- **IAC (CBI):** trayectoria 2025: 15.32% (Dic-24) → 16.39% (Sep-25) → **16.27% (cierre 4T-2025)**. Segundo trimestre
  consecutivo por encima de 16%. La leve moderación del 4T refleja que los activos ponderados por riesgo (RWA) crecieron
  algo más rápido que el capital (expansión de la cartera externa, +13.8% a/a). Finaliza el "16.34% (nov)" provisional
  de la versión previa. Mínimo regulatorio 8% (+ colchón de conservación, Acuerdo 5-2023).
- **Pérdidas no realizadas de la cartera AFS (FVOCI) del sistema — RECUPERACIÓN CASI TOTAL:** de **−US$550.9M
  (Ene-2025) a −US$34.4M (Ene-2026)** [REG]. Ver "Implicaciones contables" abajo — es el cambio más relevante de este corte.
...
### Implicaciones contables del corte marzo-2026 (nota destacada)

1. **Reversión de las pérdidas no realizadas AFS/FVOCI (el hallazgo contable del corte).** La reserva de "otras
   utilidades integrales" (ORI/OCI) por valuación de la cartera **disponible para la venta (FVOCI)** del sistema pasó de
   **−US$550.9M a −US$34.4M** en doce meses — una recuperación de **~US$516M** por la normalización de tasas y el
   repunte de precios de los bonos. Efectos: **(a)** menos lastre sobre el patrimonio y la calidad del capital (el
   patrimonio del CBI subió +1.09% a/a y las utilidades retenidas +9.52%, coherente con el IAC ≥16%); **(b)** se reduce
   el incentivo a reclasificar bonos a **costo amortizado** (held-to-maturity) para esconder la volatilidad de OCI —
   presión que en 2022-2024 empujó a varios bancos (incl. Banistmo, cuyo libro a CA subió a 43.5%) a mover papel fuera
   de FVOCI. Con la reserva ya casi plana, esa "pérdida latente" deja de ser un riesgo material de capital a nivel
   sistema. **Caveat:** es una cifra agregada del sistema; el desglose por banco no se divulga y las notas por entidad
   siguen siendo las auditadas a Dic-2025.
2. **Naturaleza del dato:** [REG] — es la contabilidad regulatoria mensual que la SBP consolida de los reportes de los
   bancos, **no auditada**. Se trata igual que un dato de gestión: sirve para la tendencia y el benchmark de ratios, no
   como cifra auditada de EE.FF. Las seis fichas por banco siguen ancladas a sus notas auditadas (Dic-2025 / jun-2025 /
   oct-2024 según el emisor).
3. **Clasificación contable (FVTPL/FVOCI/CA) por banco: sin cambio.** Al no haber EE.FF. nuevos, los pesos por categoría
   de cada banco se conservan de Dic-2025. La lectura estructural (Banistmo = el más "trading"; BG/BAC = casi todo
   FVOCI/AFS) se mantiene; la recuperación de OCI del sistema la refuerza, no la contradice.
...
**% inversiones / activo** (benchmark del sistema = SBN, banca de licencia general, **Ene-2026** [REG]; bancos a su último cierre auditado):
BG 28.6% · Davivienda 23.4% · **Sistema SBN (lic. general) 22.3%** · Banistmo 16.0% · Global 13.4% · BAC 12.3% · Scotia 9.7%.
(El benchmark del sistema es lo único que cambia en el corte marzo-2026: 22.1% Dic-25 → 22.3% Ene-26. Los seis bancos se mantienen a su cierre auditado previo.)
